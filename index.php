...
if (isset($cache_type) && $cache_type==2 && count($_POST) < 1 && $cacheRefreshTime < time()) {
    ...
    if (!isset($_SESSION['mgrValidated'])) {
        ...
        if (file_exists("{$cwd}assets/cache/{$filename}.pageCache.php")) {
            ...
            if (file_exists("{$cwd}autoload.php")) {
                $loaded_autoload = include_once("{$cwd}autoload.php");
            }
            if ($src !== false) {
                echo $src;
                exit;
...
if (!isset($loaded_autoload) && file_exists("{$cwd}autoload.php")) {
    include_once("{$cwd}autoload.php");
}
